...
def materialize(D, rules, mode="seminaive", K=100, logger=None, must_literals=None, metrics=None, fact = None):
    """
    ...
    while k < K:
        if fact != None and entailment_flag == False:
            if entail(fact,D) == True:
                # Stop the timer
                end_time = time.time()
                execution_time = end_time - start_time
                print("Entailment time:", execution_time, "seconds")
                print("Entailment: True!")
                print("Fact " + fact.__str__() + " entailed with magic after " + str(k) +" rounds of rules application!")
